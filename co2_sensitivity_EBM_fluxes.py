# This is a version to get check the EBM of CO2 sensitivity.
...
#========================================================================================================
Data_mm = []
figano = plt.figure(figsize=(18, 6))
...
for m in DataArray:
	if m == 'xpeca1':
		...
		Data_mm = ['tflhc', 'tflhd']
		count = 4
	if (m == 'xpeca1') | (m == 'xpecb1') | (m == 'xpecc1') | (m == 'tflha'):
		for mm in Data_mm:
			ax = figano.add_subplot(2, 4, count)
			textslpw, textsllpw, textshnlpw, textshslpw, textsmlpw = [], [], [], [], []
			for p in range(len(make_mapplots)):
				if make_mapplots[p]:
					var_plots[p] = ax.plot(ebm_dict[m]['zmlats'], ebm_dict[m][mm][ebmvar_name[p]],
										   label=ebmvar_name[p], color=colors_plot[p], linewidth=linewidth_list[p], linestyle=linestyle_list[p])
					textslpw.append(ma.sum(ebm_dict[m][mm][ebmvarlpw_name[p]]))
					textsllpw.append(ma.sum(ebm_dict[m][mm][ebmvarllpw_name[p]]))
					textshnlpw.append(ma.sum(ebm_dict[m][mm][ebmvarhnlpw_name[p]]))
					textshslpw.append(ma.sum(ebm_dict[m][mm][ebmvarhslpw_name[p]]))
				else:
					pass
...
plt.show()
figano.savefig('/home/bridge/nd20983/plots/EBM/EBM_expts_CO2_sensitivity.png', format='png', dpi=800)
